...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare segmentation backend reports on CMP real-eval summaries")
    parser.add_argument(
        "--entry",
        action="append",
        required=True,
    ...
    parser.add_argument("--tag", default="cmp_seg_backend_compare", help="Output file prefix")
...
def main() -> None:
    ...
    result = {
        ...
        "selection_rule": (
            "max door_presence_recall_nonempty_gt, then max avg_door_iou_nonempty_gt, "
            "then max avg_window_iou_nonempty_gt, then min avg_runtime_sec, "
            "then min avg_window_count_abs_error, then min avg_door_count_abs_error"
        ),
        "rows": rows,
    }
...
def build_row(label: str, path: Path, summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "door_presence_recall_nonempty_gt": float(summary.get("door_presence_recall_nonempty_gt", 0.0)),
...
def select_winner(rows: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    def key(row: dict[str, Any]) -> tuple[float, float, float, float, float, float]:
        return (
            float(row["door_presence_recall_nonempty_gt"]),
            float(row["avg_door_iou_nonempty_gt"]),
            float(row["avg_window_iou_nonempty_gt"]),
...
def build_markdown(rows: list[dict[str, Any]], winner: dict[str, Any] | None) -> str:
    ...
    for row in rows:
        lines.append(
            f"| {row['label']} | {row['status']} | {row['avg_window_iou_nonempty_gt']:.4f} | "
            f"{row['avg_door_iou_nonempty_gt']:.4f} | {row['door_presence_recall_nonempty_gt']:.4f} | "
            f"{row['avg_window_count_abs_error']:.3f} | {row['avg_door_count_abs_error']:.3f} | {row['avg_runtime_sec']:.4f} |"
        )
