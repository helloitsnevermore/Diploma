...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch vectorization evaluation on CMP facade real images")
    parser.add_argument(
        "--dataset-dir",
        default="data/cmp_facade_base/base",
        help="Path to CMP base folder with *.jpg and *.xml labels",
    )
    parser.add_argument("--outdir-root", default="runs/cmp_vector_eval", help="Root directory for per-image runs")
    parser.add_argument("--report-dir", default="reports", help="Directory for aggregated report files")
    parser.add_argument(
        "--report-prefix",
        default="cmp_vector_eval",
...
def build_empty_metrics(image_path: Path) -> dict[str, Any]:
    return {
        ...
        "runtime_sec": 0.0,
        "gt_windows_count": 0,
        "vector_windows_count": 0,
        "window_count_abs_error": 0,
        "window_iou": 0.0,
        "gt_doors_count": 0,
        "vector_doors_count": 0,
        "door_count_abs_error": 0,
        "door_iou": 0.0,
        "vector_line_count": 0,
        "vector_line_manhattan_ratio": 0.0,
...
def evaluate_single_run(run_dir: Path, image_path: Path) -> dict[str, Any]:
    ...
    svg_eval = parse_vector_svg(run_dir / "40_vector" / "result.svg")
    vector_windows_norm = svg_eval["windows"]
    vector_doors_norm = svg_eval["doors"]
    vector_lines = svg_eval["lines"]
    ...
    pred_window_mask = normalized_boxes_to_mask(mask_size, vector_windows_norm)
    pred_door_mask = normalized_boxes_to_mask(mask_size, vector_doors_norm)

    gt_windows_count = len(gt_windows_norm)
    gt_doors_count = len(gt_doors_norm)
    vector_windows_count = len(vector_windows_norm)
    vector_doors_count = len(vector_doors_norm)
    ...
    return {
        ...
        "vector_windows_count": int(vector_windows_count),
        "window_count_abs_error": int(abs(vector_windows_count - gt_windows_count)),
        "window_iou": float(binary_iou(pred_window_mask, gt_window_mask)),
        "gt_doors_count": int(gt_doors_count),
        "vector_doors_count": int(vector_doors_count),
        "door_count_abs_error": int(abs(vector_doors_count - gt_doors_count)),
        "door_iou": float(binary_iou(pred_door_mask, gt_door_mask)),
        "vector_line_count": int(len(vector_lines)),
        "vector_line_manhattan_ratio": float(calc_line_manhattan_ratio(vector_lines)),
        "image_path": str(image_path),
    }


def parse_vector_svg(svg_path: Path) -> dict[str, Any]:
    if not svg_path.exists():
        raise FileNotFoundError(f"Vector SVG is missing: {svg_path}")
    root = ET.fromstring(svg_path.read_text(encoding="utf-8"))

    width, height = parse_svg_size(root)
    windows_group = find_group_by_id(root, "openings_windows")
    doors_group = find_group_by_id(root, "openings_doors")
    lines_group = find_group_by_id(root, "lines")

    windows = extract_normalized_boxes(windows_group, width, height)
    doors = extract_normalized_boxes(doors_group, width, height)
    lines = extract_lines(lines_group)
    return {"windows": windows, "doors": doors, "lines": lines}
...
def parse_svg_size(root: ET.Element) -> tuple[float, float]:
    ...
    if not view_box:
        raise RuntimeError("SVG has no valid width/height or viewBox")
    parts = [p for p in view_box.replace(",", " ").split() if p]
    if len(parts) != 4:
        raise RuntimeError(f"Invalid SVG viewBox: {view_box}")
    width = float(parts[2])
    height = float(parts[3])
    if width <= 0 or height <= 0:
        raise RuntimeError(f"Invalid SVG size from viewBox: {view_box}")
    return width, height
...
def find_group_by_id(root: ET.Element, group_id: str) -> ET.Element | None:
    for elem in root.iter():
        if local_name(elem.tag) == "g" and elem.attrib.get("id") == group_id:
            return elem
    return None


def extract_normalized_boxes(group: ET.Element | None, width: float, height: float) -> list[list[float]]:
    if group is None:
        return []
    out: list[list[float]] = []
    width = max(width, 1.0)
    ...
    for elem in group:
        ...
        if name == "rect":
            x = parse_numeric(elem.attrib.get("x", "0"))
            y = parse_numeric(elem.attrib.get("y", "0"))
            w = parse_numeric(elem.attrib.get("width", "0"))
            h = parse_numeric(elem.attrib.get("height", "0"))
...
def parse_points(raw: str) -> list[tuple[float, float]]:
    ...
    for chunk in chunks:
        ...
        if len(pair) != 2:
            continue
        x = parse_numeric(pair[0])
        y = parse_numeric(pair[1])
        points.append((x, y))
    return points


def extract_lines(group: ET.Element | None) -> list[list[float]]:
    if group is None:
        return []
    out: list[list[float]] = []
    for elem in group:
        if local_name(elem.tag) != "line":
            continue
        x1 = parse_numeric(elem.attrib.get("x1", "0"))
        y1 = parse_numeric(elem.attrib.get("y1", "0"))
        x2 = parse_numeric(elem.attrib.get("x2", "0"))
        y2 = parse_numeric(elem.attrib.get("y2", "0"))
        out.append([x1, y1, x2, y2])
    return out
...
def calc_line_manhattan_ratio(lines: list[list[float]], tolerance_deg: float = 10.0) -> float:
    if not lines:
        return 0.0
    valid = 0
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    door_presence_recall = (
        mean(1.0 if int(row["vector_doors_count"]) > 0 else 0.0 for row in door_presence_rows)
    ...
    return {
        ...
        "avg_vector_line_count": mean(row["vector_line_count"] for row in ok_rows),
        "avg_vector_line_manhattan_ratio": mean(row["vector_line_manhattan_ratio"] for row in ok_rows),
    }
...
def build_markdown(rows: list[dict[str, Any]], summary: dict[str, Any]) -> str:
    ...
    if summary.get("status") == "ok":
        lines.extend(
            [
                ...
                f"- avg_vector_line_count: {summary['avg_vector_line_count']:.4f}",
                f"- avg_vector_line_manhattan_ratio: {summary['avg_vector_line_manhattan_ratio']:.4f}",
            ]
        )
    ...
    for row in rows:
        lines.append(
            f"| {row['image']} | {row['status']} | {row['window_iou']:.3f} | {row['door_iou']:.3f} | "
            f"{row['window_count_abs_error']} | {row['door_count_abs_error']} | {row['vector_line_count']} | "
            f"{row['vector_line_manhattan_ratio']:.3f} | {row['runtime_sec']:.3f} |"
        )
